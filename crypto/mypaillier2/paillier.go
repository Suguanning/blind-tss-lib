...
func GenerateKeyPair(ctx context.Context, rand io.Reader, modulusBitLen int, optionalConcurrency ...int) (privateKey *PrivateKey, publicKey *PublicKey, err error) {
	...
	var P, Q, N *big.Int
	{
		...
		N = tmp.Mul(P, Q)
		N = N.Mul(N, tss.EC().Params().N)
	}

	// phiN = P-1 * Q-1*(ec.p - 1)
	PMinus1, QMinus1, ECPMinus1 := new(big.Int).Sub(P, one), new(big.Int).Sub(Q, one), new(big.Int).Sub(tss.EC().Params().N, one)
	phiN := new(big.Int).Mul(PMinus1, QMinus1)
	phiN = phiN.Mul(phiN, ECPMinus1)
...
func (privateKey *PrivateKey) Decrypt(c *big.Int) (m *big.Int, err error) {
	...
	//1. D = c^{(p-1)(q-1)(ecc.p-1))} mod N2
	D := new(big.Int).Exp(c, PhiN, N2)
	//2. LeftTerm = [(D-1)/(Npq)]
	DMinus1 := new(big.Int).Sub(D, one)
	Npq := new(big.Int).Mul(privateKey.N, privateKey.P)
	Npq = Npq.Mul(Npq, privateKey.Q)
	LeftTerm := DMinus1.Div(DMinus1, Npq)
	//LeftTerm = LeftTerm.Mod(LeftTerm, tss.EC().Params().N)
	//3. RightTerm = (p-1)(q-1)(ecc.p-1)^{-1}
	RightTerm := modQ.ModInverse(PhiN)
	m = modQ.Mul(LeftTerm, RightTerm)
	return
}
